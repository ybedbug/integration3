                            ...
                            size_t hdr_size)
{
    ...
    ucs_assertv(max_payload > 0,
                "length=%zu weight=%zu%% max_frag=%zu hdr_size=%zu",
                         ...
                         unsigned dt_mask)
{
    ...
    ucs_assertv(req->send.multi_lane_idx < mpriv->num_lanes,
                ...
                mpriv->num_lanes);
    ucs_assertv(!ucp_datatype_iter_is_end(&req->send.state.dt_iter),
                "offset %zu length %zu", req->send.state.dt_iter.offset,
                req->send.state.dt_iter.length);
