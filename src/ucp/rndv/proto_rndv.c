...
ucp_proto_rndv_ctrl_init(const ucp_proto_rndv_ctrl_init_params_t *params)
{
    ...
    *params->super.super.priv_size = sizeof(ucp_proto_rndv_ctrl_priv_t);
    ...
    ucs_linear_func_add_inplace(&send_overheads, params->unpack_time);
    ...
    do {
        ...
        ucs_trace("range[%d] ..%zu"
                  " send_overheads:" UCP_PROTO_PERF_FUNC_FMT
                  " remote-single:" UCP_PROTO_PERF_FUNC_FMT
                  " remote-multi:" UCP_PROTO_PERF_FUNC_FMT
                  " --> single:" UCP_PROTO_PERF_FUNC_FMT
                  " multi:" UCP_PROTO_PERF_FUNC_FMT,
                  caps->num_ranges, perf_range->max_length,
                  UCP_PROTO_PERF_FUNC_ARG(&send_overheads),
                                     ...
                                     ucs_linear_func_t *ack_perf)
{
    ...
    ucp_proto_rndv_get_ack_time(init_params, apriv->lane, ack_perf);
    return UCS_OK;
}
                          ...
                          size_t rkey_length)
{
    ...
    if (rkey_length > 0) {
        ...
        status = ucp_ep_rkey_unpack_internal(ep, rkey_buffer,
                                             rkey_length, &rkey);
        if (status != UCS_OK) {
            return status;
        }
                                  ...
                                  const void *rkey_buffer, size_t rkey_length)
{
    ...
    recv_req->recv.remaining      = send_length; // TODO is this needed?
#if ENABLE_DEBUG_DATA
    recv_req->recv.rndv_req       = req;
#endif
    ucs_assert(recv_req->recv.rndv_req != NULL);
...
ucp_proto_rndv_handle_rtr(void *arg, void *data, size_t length, unsigned flags)
{
    ...
    if (rtr->size == req->send.state.dt_iter.length) {
        ...
        freq->flags  |= UCP_REQUEST_FLAG_CALLBACK;
