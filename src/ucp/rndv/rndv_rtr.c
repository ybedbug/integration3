                               ...
                               ucs_sys_device_t sys_dev)
{
    ...
    ucp_proto_rndv_ctrl_init_params_t params = {
        ...
        .super.memtype_op    = UCT_EP_OP_LAST,//TODO
...
static ucs_status_t ucp_proto_rndv_rtr_mtype_progress(uct_pending_req_t *self)
{
    ...
    return ucp_proto_rndv_rtr_common_send(req, ucp_proto_rndv_rtr_mtype_pack);
}
...
ucp_proto_rndv_rtr_mtype_init(const ucp_proto_init_params_t *init_params)
{
    ...
    status = ucp_proto_rndv_mtype_init(init_params, NULL, &frag_size);
    if (status != UCS_OK) {
        return status;
    }
    ...
    ucs_trace("rtr unpack " UCP_PROTO_PERF_FUNC_FMT,
              UCP_PROTO_PERF_FUNC_ARG(&unpack_time));

    return ucp_proto_rndv_rtr_common_init(
            init_params, ucp_proto_rndv_rtr_mtype_data_received,
